...
async function createDpopProof(url, method, accessToken = null) {
  ...
  const accessTokenHash = accessToken
    ? await CRYPT.createHash(accessToken, true)
    : null;
...
function captureDpopNonce(response) {
  ...
  if (nonce) {
    localStorage.setItem(LS.DPOP_NONCE, nonce);
  }
}
...
async function step02GetDidDocument(did) {
  ...
  if (!resp.ok) {
    throw new Error(`Failed to fetch DID document (HTTP ${resp.status}).`);
  }
  return resp.json();
}
...
async function step03GetPdsMetadata(pdsUrl) {
  ...
  if (!resp.ok) {
    throw new Error(`Failed to fetch PDS metadata (HTTP ${resp.status}).`);
  }
  return resp.json();
}
...
async function step04GetAuthServerDiscovery(authServerUrl) {
  ...
  if (!resp.ok) {
    throw new Error(
      `Failed to fetch auth server discovery (HTTP ${resp.status}).`,
    );
  }
  return resp.json();
}
...
async function step05PARRequest(parEndpoint, handle, redirectUri) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    const err = await resp.json().catch(() => ({}));
    if (attempt === 0 && err.error === "use_dpop_nonce") {
      continue;
    }
    throw new Error(
      err.error_description || `PAR request failed (HTTP ${resp.status}).`,
    );
  }
}
...
export async function login(handle) {
  ...
  localStorage.setItem(
    LS.REVOCATION_EP,
    authServerDiscovery.revocation_endpoint || "",
  );
  ...
  step06RedirectToAuthPage(
    authServerDiscovery.authorization_endpoint,
    requestUri,
  );
}
...
export async function exchangeCodeForToken() {
  ...
  if (!resp.ok) {
    const err = await resp.json().catch(() => ({}));
    throw new Error(
      err.error_description || `Token exchange failed (HTTP ${resp.status}).`,
    );
  }
